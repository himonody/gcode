...
// 6. 定时消息：使用 Nats-Schedule 头部实现定时投递
// 消息在指定的时间点才会被投递给消费者
func main() {
	const (
		url     = "nats://127.0.0.1:4222" // NATS 服务器地址
		stream  = "F_SCHEDULE"            // JetStream 流名称
		subject = "features.schedule"     // 消息主题
		durable = "F_SCHED_D"             // 持久化消费者名称
	)
	...
	// 发布定时消息（使用 Nats-Schedule 头部指定投递时间，需要 NATS 2.11+）
	// 注意：需要在流上启用调度器功能
	fireAt := time.Now().Add(10 * time.Second)
	msg := &nats.Msg{Subject: subject, Data: []byte("定时消息内容"), Header: nats.Header{}}
	msg.Header.Set("Nats-Schedule", "@at "+fireAt.UTC().Format(time.RFC3339)) // 设置投递时间
	msg.Header.Set("Nats-Schedule-Target", subject)                           // 设置目标主题
	log.Printf("发布定时消息，投递时间 %s（当前时间=%s）", fireAt.Format(time.RFC3339), time.Now().Format(time.RFC3339))
	if _, err := js.PublishMsg(msg); err != nil {
		log.Printf("发布失败（可能未启用调度器）: %v", err)
	}

	log.Println("定时消息演示运行中；按 Ctrl+C 退出")
	<-ctx.Done()
	log.Println("正在关闭")
}
